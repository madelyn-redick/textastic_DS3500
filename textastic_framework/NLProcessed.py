...
class NLProcessed:
    ...
    def sankey_ngrams(self, k=10):
        ...
        # formats the ngrams into a dictionary (assumes ngrams are bigrams although code can be latered altered to
    ...
    def sentence_sentiment(self, columns=5, rows=2, hist=True):
        ...
        if hist:
            fig, axs = plt.subplots(rows, columns, figsize=(columns, rows), sharey=True, num='SENTIMENT SUBPLOTS - DENSITY')
        else:
